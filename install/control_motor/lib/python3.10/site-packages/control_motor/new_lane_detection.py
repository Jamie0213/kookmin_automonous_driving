...
- 추가: x_location 퍼블리시(/x_location, std_msgs/Float32)
"""
...
from new_slidingwindow import SlideWindow
from new_utils import *   # roi_for_lane, process_image, warper 등

# ───── 파라미터(주행) ─────
CENTER_X      = 320
STEER_LIMIT   = 1000.0
DEFAULT_SPEED = 35.0  #35
CURVE_SPEED   = 30.0
DEFAULT_GAIN  = 0.25 
CURVE_GAIN    = 0.60
LANE_MIN_PIX  = 500
...
class LaneDetectionROS(Node):
    ...
    def __init__(self):
        ...
        self.pub_xloc  = self.create_publisher(Float32, "/x_location", 10)  # 추가: x_location 퍼블리셔

        # 상태
        self.cv_image: np.ndarray | None = None
        self.lane_msg  = Int32()
        self.motor_msg = Float32MultiArray()
        self.xloc_msg  = Float32()
        ...
        self.declare_parameter('debug_windows', "01_Orig,08_Slide")
    ...
    def process_frame(self):
        ...
        # control + x_location publish
        if x_loc is None:
            x_loc = CENTER_X
        err_px = x_loc - CENTER_X
        gain   = DEFAULT_GAIN if abs(err_px) < 20 else CURVE_GAIN
        angle  = float(np.clip((err_px) * gain, -STEER_LIMIT, STEER_LIMIT))
        speed  = DEFAULT_SPEED if abs(err_px) < 20 else CURVE_SPEED

        # 로그 및 x_location 퍼블리시
        #self.get_logger().info(f"x_location={x_loc}")
        self.xloc_msg.data = float(x_loc)
        self.pub_xloc.publish(self.xloc_msg)
...
if __name__ == "__main__":
    main()
# 각종 부드럽게 이동하는 로직 적용하기 전의 휘청휘청버전 + x_location 퍼블리시 추가
