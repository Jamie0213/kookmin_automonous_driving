...
def warper(image):
    ...
    # src_point1 = [136, 159]  # 왼쪽 아래 점
    # src_point4 = [575, 151]  # 오른쪽 아래 점
    ...
    ####### kuac #####
    src_point1 = [0, 124]  # 왼쪽 아래 점
    src_point2 = [165, 56]  # 왼쪽 위 점
    src_point3 = [515, 28]  # 오른쪽 위 점
    src_point4 = [640, 75]  # 오른쪽 아래 점
    ...
    dst_point1 = [x // 4 -25 , y]  # 변환 이미지에서의 왼쪽 아래 점
    dst_point2 = [x // 4 -25, 0]  # 변환 이미지에서의 왼쪽 위 점
    dst_point3 = [x // 4 * 3 -25 , 0]  # 변환 이미지에서의 오른쪽 위 점
    dst_point4 = [x // 4 * 3  -25, y]  # 변환 이미지에서의 오른쪽 아래 점
...
#     ####### kuac #####
#     src_point1 = [0, 124]   # 왼쪽 아래 점
#     src_point2 = [155, 56]  # 왼쪽 위 점
#     src_point3 = [515, 28]  # 오른쪽 위 점
#     src_point4 = [640, 75]  # 오른쪽 아래 점

#     # ※ 점 순서는 좌하→좌상→우상→우하(시계/반시계 일관)로 맞추는 게 안정적입니다.
#     src_points = np.float32([src_point1, src_point2, src_point3, src_point4])  # 원본 이미지에서의 점들
...
#     # [ADD] ──────────────────────── 소스 마스크(워프 전, ROI 밖 제거)
#     src_mask = np.zeros((y, x), dtype=np.uint8)
#     cv2.fillConvexPoly(src_mask, src_points.astype(np.int32), 255)
#     image_masked = cv2.bitwise_and(image, image, mask=src_mask)

#     matrix = cv2.getPerspectiveTransform(src_points, dst_points)  # 원근 변환 행렬 계산

#     # [ADD] 리샘플링/보더 설정 (ROI 밖은 0으로)
#     warped_img = cv2.warpPerspective(
#         image_masked, matrix, (x, y),
#         flags=cv2.INTER_LINEAR,                       # 축소가 크면 INTER_AREA도 고려 가능
#         borderMode=cv2.BORDER_CONSTANT, borderValue=0
#     )

#     # [ADD] ──────────────────────── 목적지 마스크(워프 후, dst 폴리곤 밖 제거)
#     dst_mask = np.zeros((y, x), dtype=np.uint8)
#     cv2.fillConvexPoly(dst_mask, dst_points.astype(np.int32), 255)
#     warped_img = cv2.bitwise_and(warped_img, warped_img, mask=dst_mask)
